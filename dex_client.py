...
CHAIN_RPC_ENV = {"eth":"ETH_RPC_URL_PRIMARY","bsc":"BSC_RPC_URL_PRIMARY","polygon":"POLYGON_RPC_URL_PRIMARY","base":"BASE_RPC_URL_PRIMARY","arb":"ARB_RPC_URL_PRIMARY","op":"OP_RPC_URL_PRIMARY","avax":"AVAX_RPC_URL_PRIMARY","ftm":"FTM_RPC_URL_PRIMARY"}
...
def _normalize_market(ds: Dict[str, Any]) -> Dict[str, Any]:
    m = {"pairSymbol": f"{ds.get('baseToken',{}).get('symbol','?')}/{ds.get('quoteToken',{}).get('symbol','?')}","chain": ds.get("chainId") or ds.get("chain") or "—","price": float(ds.get("priceUsd") or 0) or None,"fdv": ds.get("fdv"), "mc": ds.get("marketCap"),"liq": ds.get("liquidity",{}).get("usd"), "vol24h": ds.get("volume",{}).get("h24"),"priceChanges": { "m5": ds.get("priceChange",{}).get("m5"),"h1": ds.get("priceChange",{}).get("h1"),"h24": ds.get("priceChange",{}).get("h24") },"ageDays": None, "pairAddress": ds.get("pairAddress"),"baseAddress": ds.get("baseToken",{}).get("address"),"quoteAddress": ds.get("quoteToken",{}).get("address"),"tokenAddress": ds.get("baseToken",{}).get("address"),"sources": ["DexScreener"],"source": "DexScreener","links": {},"asof": time.strftime("%Y-%m-%d %H:%M UTC", time.gmtime())}
    if ds.get("age"):
...
def _ds_by_token(chain: str, token: str) -> Dict[str, Any]:
    ...
    for p in pairs:
        if (p.get("chainId") or p.get("chain")) == chain:
            m = _normalize_market(p); m["ok"] = True; m["tokenAddress"] = token; return m
    m = _normalize_market(pairs[0]); m["ok"] = True; m["tokenAddress"] = token; return m
