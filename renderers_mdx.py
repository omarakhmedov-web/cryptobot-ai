...
def render_details(verdict, market: Dict[str, Any], ctx: Dict[str, Any], lang: str = "en") -> str:
    ...
    # Determine domain to probe (prefer ctx['rdap'] if local _rd missing)
    _rd_local = _rd if isinstance(_rd, dict) else ((ctx or {}).get("rdap") if isinstance((ctx or {}).get("rdap"), dict) else {})
    _domain_to_probe = _resolve_domain(_rd_local or {}, market, ctx)

    # Domain gating: avoid cross-card bleed
    ...
    # WHOIS fallbacks with domain gating
    def _pick(*vals):
        for v in vals:
    ...
    if _dom_match:
        ...
        who_created = who.get("created")
        who_registrar = who.get("registrar")

    # If still missing, reuse local RDAP (_rd) or ctx['rdap'] when domain matches
    ...
    # Additional fallback: ctx['whois'] (not gated strictly because values may be generic)
