...
def _render_report(addr: str, text: str):
    """
    Build HTML report with a SINGLE source of truth:
      - Run final text through risk gates and sanitizers
      - Extract Risk score & Trust verdict from that final text
      - Derive Signals from the "Why++ factors" block (negatives/positives)
    """
    import re, tempfile
    from datetime import datetime

    # 0) Enrich long form (keeps domains/links etc. as before)
    try:
        text = _enrich_full(addr, text)
    except Exception:
        pass

    # 1) Apply same gates/sanitizers the chat uses — ensures parity
    def _maybe(fn, s):
        try:
            return fn(s)
        except Exception:
            return s
    try:
        if '_apply_risk_gates__text' in globals():
            text = _apply_risk_gates__text(text)
    except Exception:
        pass
    text = _maybe(_normalize_escapes, text) if '._' else text
    text = _maybe(_dedupe_verdict_and_risk_strict, text)

    # 2) Parse final score & verdict from the (already gated) text
    verdict = None
    score = None
    try:
        m = re.search(r'(?mi)^\s*Trust\s+verdict\s*:\s*(.+)$', text)
        if m:
            verdict = m.group(1).strip()
        ms = re.search(r'(?mi)Risk\s*score\s*:\s*(\d+)\s*/\s*100', text)
        if ms:
    ...
    # 3) Extract Why++ blocks for Signals/Positives
    neg_lines, pos_lines = [], []
    try:
        mstart = re.search(r'(?mi)^\s*Why\+\+\s*factors\s*$', text)
        if mstart:
            tail = text[mstart.end():]
            mend = re.search(r'(?mi)^\s*(On-chain|ℹ️|🔒|Scan token:|$)', tail)
            block = tail[:mend.start()] if mend else tail
            for ln in block.splitlines():
                s = ln.strip()
                if not s:
                    continue
                mneg = re.match(r'^[−\-]\s*(\d+)\s+(.+)$', s)
                mpos = re.match(r'^[\+]\s*(\d+)\s+(.+)$', s)
                if mneg:
                    neg_lines.append(f"− {mneg.group(1)}  {mneg.group(2).strip()}")
                elif mpos:
                    pos_lines.append(f"+ {mpos.group(1)}  {mpos.group(2).strip()}")
    except Exception:
        pass
    ...
    # 4) Pull domain/pair/dex/chain hints from text header (best‑effort)
    dom = "—"
    pair = dex = chain = None
    try:
        mdom = re.search(r'(?mi)^Domain:\s*(\S+)', text)
        if mdom:
            dom = mdom.group(1).strip()
...
<div class="box"><h2>Risk verdict</h2><p><b>""" + verdict + f""" (Risk score: {score}/100)</b></p>
<h3>Signals</h3><pre>""" + _lines(neg_lines) + """</pre><h3>Positives</h3><pre>""" + _lines(pos_lines) + """</pre></div>
