...
APP_VERSION = os.environ.get("APP_VERSION", "0.4.4a-wl+why-hardened")
    ...
    try:
        ...
        try:
            ...
            return dt.strftime("%Y-%m-%d")
                        ...
                        try:
                            payload = data.split(":",1)[1].strip().lower()
                            addr = payload if ADDR_RE.fullmatch(payload) else (msg2addr.get(msg_id) or _extract_addr_from_text(msg_obj.get("text") or ""))
                            if not addr:
                                tg_answer_callback(TELEGRAM_TOKEN, cq["id"], "address?", logger=app.logger); return ("ok", 200)
                            det = RISK_CACHE.get(addr, None)
                            if not det:
                                s, lbl, rs = _risk_verdict(addr, msg_obj.get("text") or "")
                                det = {"score": s, "label": lbl, "neg": rs.get("neg", []), "pos": rs.get("pos", [])}
                            lines = [f"Why: {det['label']} (score {det['score']}/100)"]
                            if det.get("neg"): lines.append("⚠️ Signals: " + "; ".join(det["neg"]))
                            if det.get("pos"): lines.append("✅ Positives: " + "; ".join(det["pos"]))
                            try:
                                lines.append(f"Thresholds: CAUTION≥{RISK_THRESH_CAUTION}, HIGH≥{RISK_THRESH_HIGH}; Liq<${int(RISK_LIQ_LOW):,}/{int(RISK_LIQ_MED):,}, Vol<${int(RISK_VOL_LOW):,}")
                            except Exception:
                                pass
                            why_text = "\n".join(lines)
                            _send_text(chat_id, NEWLINE_ESC_RE.sub("\n", why_text), logger=app.logger)
                            tg_answer_callback(TELEGRAM_TOKEN, cq["id"], "why", logger=app.logger)
                        except Exception as e:
