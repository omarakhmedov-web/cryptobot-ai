...
# === Minimal resilient fetcher for singles (keeps Top‑10 untouched) ===
def _get_prices_resilient_min(ids: list[str], vs: str = "usd") -> dict:
    ...
    # 2) CoinGecko simple (batch) — для хвостов
    ...
    # 3) Coinbase tails (точечно)
    tails = [i for i in ids if i not in out]
    for cid in tails:
        if cid in ("tether","usd-coin"):
        ...
        p = _coinbase_price(sym)
        if p is not None:
            out[cid] = {"usd": float(p), "last_updated_at": now_ts}
