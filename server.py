...
def on_callback(cb):
    ...
    # === WATCHLIST (fixed, plain text) ===
    if action == "WATCHLIST":
        import os, json
        _chat_id = chat_id if 'chat_id' in locals() else (cb.get("message") or {}).get("chat", {}).get("id")
        _cb_id = cb.get("id")
        db_path = os.environ.get("WATCH_DB_PATH", "./watch_db.json")
        items = []
        try:
            with open(db_path, "r", encoding="utf-8") as _f:
                _dataj = json.load(_f)
            if isinstance(_dataj, dict):
                _raw = _dataj.get(str(_chat_id)) or _dataj.get(int(_chat_id)) or _dataj.get("tokens") or []
                if isinstance(_raw, dict) and "tokens" in _raw:
                    items = list(_raw.get("tokens") or [])
                elif isinstance(_raw, list):
                    items = list(_raw)
            elif isinstance(_dataj, list):
                items = list(_dataj)
        except Exception:
            items = []
        ...
        if not items:
            try:
                answer_callback_query(_cb_id, "Watchlist is empty.", False)
            except Exception:
                pass
            try:
                send_message(_chat_id, "Your watchlist is empty.\nAdd tokens with /watch 0x...", disable_web_page_preview=True)
            except Exception:
                pass
            return jsonify({"ok": True})
        ...
        for _x in items:
            ...
            if _t.startswith("0x") and len(_t) > 14:
                _t = _t[:10] + "…" + _t[-6:]
            _norm.append(_t)
        _lines = [f"{i+1}) {t}" for i, t in enumerate(_norm[:50])]
        _text = "Your Watchlist\n" + "\n".join(_lines)
        send_message(_chat_id, _text, disable_web_page_preview=True)
        try:
            answer_callback_query(_cb_id, "Watchlist sent.", False)
        except Exception:
            pass
        return jsonify({"ok": True})
    if orig_msg_id == 0:
