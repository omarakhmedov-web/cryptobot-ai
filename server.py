...
# --------- Короткие callback-токены (Fix Button_data_invalid) ---------
...
def send_donate_message(chat_id: int, lang: str):
    ...
    bot.send_message(chat_id=chat_id, text=texts.get(lang, texts["en"]),
                     reply_markup=build_donate_keyboard(), parse_mode="Markdown",
                     disable_web_page_preview=True)

def send_qr(chat_id: int, label: str, value: str):
    img = qrcode.make(value)
    bio = io.BytesIO(); bio.name = f"{label}.png"
...
def analyze_eth_contract(address: str) -> dict:
    ...
    if not res.get("ok"):
        facts["error"] = res.get("error"); return facts
    info = (res["data"] or [{}])[0]
...
def format_report(facts: dict, lang: str) -> str:
    ...
    if "error" in facts and not facts.get("abi_present"):
        return L["error"]
    lines = [
        f"🧭 {L['network']}: {facts.get('network')}",
        f"🔗 {L['address']}: {facts.get('address')}",
    ]
    ...
    if funcs: lines.append(f"🧰 {L['funcs']}: " + ", ".join(funcs))
    return "\n".join(lines)

# -------------------- Fresh Web Search --------------------
FRESH_TRIGGERS = re.compile(r"\b(today|now|latest|news|price|prices|update|updated|2024|2025|rate|inflation|btc|eth|ton|market)\b", re.IGNORECASE)
...
def serpapi_search(query: str, lang: str) -> list:
    ...
    try:
        params = {"engine":"google","q":query,"api_key":SERPAPI_KEY,"hl":"en" if lang=="en" else ("ru" if lang=="ru" else "ar"),"num":"5"}
        ...
        app.logger.warning(f"serpapi_search error: {e}"); return []
...
def duckduckgo_fallback(query: str) -> list:
    try:
        ...
        html_text = resp.text; results=[]
        link_pat = re.compile(r'<a[^>]+class="result__a"[^>]+href="([^"]+)"[^>]*>(.*?)</a>', re.I|re.S)
        snip_pat = re.compile(r'<a[^>]+class="result__snippet"[^>]*>(.*?)</a>', re.I|re.S)  # FIX: re.S
        links = link_pat.findall(html_text)[:5]; snips = snip_pat.findall(html_text)[:5]
        for i, (href, title_html) in enumerate(links):
            title = html.unescape(re.sub("<.*?>", "", title_html)).strip()
            snippet = html.unescape(re.sub("<.*?>", "", snips[i])).strip() if i < len(snips) else ""
            results.append({"title": title, "link": href, "snippet": snippet})
        return results
    except Exception as e:
        app.logger.warning(f"duckduckgo_fallback error: {e}"); return []

def compose_snippets_text(snips: list, lang: str) -> str:
    if not snips: return ""
    date_str = datetime.utcnow().strftime("%Y-%m-%d")
    header = {"en": f"Fresh web snippets (UTC {date_str}):","ru": f"Свежие сниппеты из веба (UTC {date_str}):","ar": f"ملخصات حديثة من الويب (UTC {date_str}):"}.get(lang, f"Fresh web snippets (UTC {date_str}):")
    lines = [header]
    for s in snips:
...
PRICE_TRIGGERS = re.compile(
    ...
    r"|(?:\b|_)(?:сейчас|на\s+данный\s+момент|прямо\s+сейчас|now|at\s+the\s+moment)\b", re.IGNORECASE)
SYMBOL_TO_CG = {
...
TICKER_RE = re.compile(r"(?:(?<=\$)|\b)([A-Z]{2,6}|btc|eth|sol|ton|usdt|usdc|bnb|arb|op|ada|xrp|avax|trx|doge|matic|sui|apt)\b", re.IGNORECASE)
def is_price_query(text: str) -> bool:
    return bool(text) and (bool(PRICE_TRIGGERS.search(text)) or bool(TICKER_RE.search(text)))

def _cg_ids_from_text(text: str) -> list[str]:
    t = (text or "").lower(); ask_all = any(w in t for w in ("все","всё","all"))
    default_top = ["bitcoin","ethereum","solana","the-open-network","tether","usd-coin","binancecoin","ripple","cardano","dogecoin"]
    syms = set(m.group(1).upper() for m in TICKER_RE.finditer(text or ""))
    if ask_all and not syms: return default_top
    if not syms: return ["bitcoin","ethereum","solana","the-open-network"]
    ids, seen = [], set()
    for s in syms:
...
def _cg_cache_get(key: str):
    if time.time() - _cg_cache["t"] < 60 and _cg_cache["key"] == key: return _cg_cache["data"]
    return None
def _cg_cache_set(key: str, data: dict): _cg_cache.update({"t":time.time(),"key":key,"data":data})

def coingecko_prices(coin_ids: list[str], vs="usd") -> dict:
    coin_ids = [c for c in coin_ids if c] or ["bitcoin","ethereum"]
    coin_ids_str = ",".join(coin_ids); cache_key = f"{coin_ids_str}:{vs}"
    ...
    try:
        ...
        _cg_cache_set(cache_key, data); return data
    except Exception as e:
        return {"error": str(e)}
...
def format_prices_message(data: dict, lang: str = "en", vs="usd") -> str:
    ...
    name_map = {"bitcoin":"BTC","ethereum":"ETH","solana":"SOL","the-open-network":"TON","tether":"USDT","usd-coin":"USDC","binancecoin":"BNB","arbitrum":"ARB","optimism":"OP","cardano":"ADA","ripple":"XRP","avalanche-2":"AVAX","tron":"TRX","dogecoin":"DOGE","matic-network":"MATIC","sui":"SUI","apt":"APT"}
    ...
    for k in order + [k for k in data.keys() if k not in order]:
        ...
        item = data[k]; price = item.get(vs); if price is None: continue
        sym = name_map.get(k, k); chg = item.get(f"{vs}_24h_change"); chg_s=""
        if isinstance(chg,(int,float)):
            sign = "▲" if chg>=0 else "▼"; chg_s = f"  {sign}{abs(chg):.2f}%/24h"
        lines.append(f"{sym}: ${price:,.4f}{chg_s}")
    if len(lines)==1: return {"en":"No price data.","ru":"Нет данных по ценам.","ar":"لا توجد بيانات أسعار."}.get(lang, "No price data.")
    try:
...
def coingecko_top_market(cap_n: int = 10) -> list[dict]:
    try:
        ...
        params = {"vs_currency":"usd","order":"market_cap_desc","per_page":str(cap_n),"page":"1","price_change_percentage":"24h"}
        r = requests.get(url, params=params, timeout=15, headers={"User-Agent":"Mozilla/5.0"})
        r.raise_for_status(); return r.json() or []
    except Exception as e:
        app.logger.warning(f"coingecko_top_market error: {e}"); return []

def format_top10(mkts: list[dict], lang: str = "en") -> tuple[str, list[str]]:
    if not mkts:
        return ({"en":"No market data.","ru":"Нет рыночных данных.","ar":"لا توجد بيانات سوق."}.get(lang, "No market data."), [])
    lines = {"en":["🏆 Top-10 by market cap (USD):"],"ru":["🏆 Топ-10 по капитализации (USD):"],"ar":["🏆 أعلى 10 بالقيمة السوقية (USD):"]}.get(lang, ["🏆 Top-10 by market cap (USD):"])
    ids=[]
    for i,c in enumerate(mkts, start=1):
        sym = (c.get("symbol") or "").upper(); price = c.get("current_price"); chg = c.get("price_change_percentage_24h"); chg_s=""
        if isinstance(chg,(int,float)): sign = "▲" if chg>=0 else "▼"; chg_s = f"  {sign}{abs(chg):.2f}%/24h"
        lines.append(f"{i}. {sym}: ${price:,.4f}{chg_s}"); ids.append(c.get("id"))
...
# -------------------- GAS / FEARGREED / BTC DOM --------------------
def fetch_gas_etherscan() -> dict | None:
    if not ETHERSCAN_API_KEY: return None
    try:
        r = requests.get("https://api.etherscan.io/api", params={"module":"gastracker","action":"gasoracle","apikey":ETHERSCAN_API_KEY}, timeout=10)
        j = r.json(); res = j.get("result") or {}
        return {"source":"etherscan","safe":float(res.get("SafeGasPrice")), "propose":float(res.get("ProposeGasPrice")), "fast":float(res.get("FastGasPrice")), "base":float(res.get("suggestedBaseFee", 0))}
    except Exception: return None

def fetch_gas_ethgasstation() -> dict | None:
    try:
        r = requests.get("https://ethgasstation.info/json/ethgasAPI.json", timeout=10, headers={"User-Agent":"Mozilla/5.0"})
        j = r.json()
        return {"source":"ethgasstation","safe":float(j.get("safeLow",0))/10.0,"propose":float(j.get("average",0))/10.0,"fast":float(j.get("fast",0))/10.0,"base":float(j.get("average",0))/10.0}
    except Exception: return None

def fetch_gas_etherchain() -> dict | None:
    try:
        r = requests.get("https://etherchain.org/api/gasnow", timeout=10, headers={"User-Agent":"Mozilla/5.0"})
        j = r.json().get("data", {})
        to_gwei = lambda wei: float(wei)/1e9 if wei is not None else None
        return {"source":"etherchain","safe":to_gwei(j.get("slow")), "propose":to_gwei(j.get("standard")), "fast":to_gwei(j.get("rapid")), "base":to_gwei(j.get("standard"))}
    except Exception: return None

def get_eth_gas() -> dict:
    for fn in (fetch_gas_etherscan, fetch_gas_ethgasstation, fetch_gas_etherchain):
        data = fn()
        if data and data.get("propose"): return data
...
def format_gas_message(data: dict, lang: str) -> str:
    ...
    lines = {"en":["⛽ Ethereum gas (gwei):"],"ru":["⛽ Газ Ethereum (gwei):"],"ar":["⛽ غاز إيثريوم (gwei):"]}.get(lang, ["⛽ Ethereum gas (gwei):"])
    lines += [f"Safe: {data.get('safe'):.1f}", f"Propose: {data.get('propose'):.1f}", f"Fast: {data.get('fast'):.1f}"]
    if data.get("base") is not None: lines.append(f"Base fee: {data.get('base'):.1f}")
    dt = datetime.utcnow().strftime("%Y-%m-%d %H:%M UTC")
    lines.append({"en":f"\nSource: {src}. As of {dt}.","ru":f"\nИсточник: {src}. По состоянию на {dt}.","ar":f"\nالمصدر: {src}. حتى {dt}."}.get(lang, f"\nSource: {src}. As of {dt}."))
    return "\n".join(lines)
...
def fetch_fear_greed() -> dict:
    try:
        ...
        j = r.json(); item = (j.get("data") or [{}])[0]
        return {"value": item.get("value"), "classification": item.get("value_classification"), "timestamp": item.get("timestamp")}
    except Exception: return {"error":"fng_unavailable"}

def format_fear_greed(d: dict, lang: str) -> str:
    if "error" in d or not d.get("value"):
        return {"en":"Fear & Greed data unavailable.","ru":"Индекс страха и жадности недоступен.","ar":"بيانات مؤشر الخوف والطمع غير متاحة."}.get(lang, "Fear & Greed data unavailable.")
    ...
    try:
        ...
        dt = datetime.utcnow().strftime("%Y-%m-%d %H:%M UTC")
    hdr = {"en":"😨/😎 Crypto Fear & Greed Index:","ru":"😨/😎 Индекс страха и жадности:","ar":"😨/😎 مؤشر الخوف والطمع:"}.get(lang, "😨/😎 Crypto Fear & Greed Index:")
...
def fetch_btc_dominance() -> dict:
    ...
    except Exception: return {"error":"btcdom_unavailable"}

def format_btc_dominance(d: dict, lang: str) -> str:
    if "error" in d or d.get("dominance") is None:
        return {"en":"BTC dominance unavailable.","ru":"Доминация BTC недоступна.","ar":"هيمنة BTC غير متاحة."}.get(lang, "BTC dominance unavailable.")
    dom = float(d["dominance"]); mcap = d.get("mcap_usd")
    lines = {"en":[f"🟧 BTC dominance: {dom:.2f}%"],"ru":[f"🟧 Доминация BTC: {dom:.2f}%"],"ar":[f"🟧 هيمنة BTC: {dom:.2f}%"]}.get(lang, [f"🟧 BTC dominance: {dom:.2f}%"])
    if isinstance(mcap,(int,float)):
        lines.append({"en":f"Total crypto mcap: ${mcap:,.0f}","ru":f"Общая капитализация рынка: ${mcap:,.0f}","ar":f"القيمة السوقية الإجمالية: ${mcap:,.0f}"}[lang])
    dt = datetime.utcnow().strftime("%Y-%m-%d %H:%M UTC")
    lines.append({"en":f"\nAs of {dt}.","ru":f"\nПо состоянию на {dt}.","ar":f"\nحتى {dt}."}.get(lang, f"\nAs of {dt}."))
    return "\n".join(lines)
...
def ai_reply(user_text: str, lang: str, chat_id: int) -> str:
    try:
        ...
        if needs_fresh_search(user_text):
            snips = serpapi_search(user_text, lang) or duckduckgo_fallback(user_text)
            snippets_text = compose_snippets_text(snips, lang)
            if snippets_text: msgs.append({"role":"system","content":snippets_text})
        msgs.append({"role":"user","content":user_text})
        resp = client.chat.completions.create(model=MODEL, messages=msgs, temperature=0.15, max_tokens=650)
        content = (resp.choices[0].message.content or "").strip()
        remember(chat_id, "user", user_text); remember(chat_id, "assistant", content)
        return content
    except Exception as e:
...
def webhook():
    ...
    if "callback_query" in update:
        cq = update["callback_query"]; data = cq.get("data") or ""
        ...
        try:
            if data == "qr_eth":
                ...
                try:
                    bot.edit_message_text(chat_id=chat_id, message_id=cq.get("message",{}).get("message_id"),
                                          text=msg_now, reply_markup=build_price_keyboard(chat_id, ids, lang_cq))
                except Exception:
                ...
                lang_cq = get_lang_override(chat_id) or DEFAULT_LANG
                msg = format_gas_message(get_eth_gas(), lang_cq)
                try:
                    bot.edit_message_text(chat_id=chat_id, message_id=cq.get("message",{}).get("message_id"),
                                          text=msg, reply_markup=build_gas_keyboard(lang_cq))
                except Exception:
                ...
                lang_cq = get_lang_override(chat_id) or DEFAULT_LANG
                msg = format_fear_greed(fetch_fear_greed(), lang_cq)
                try:
                    bot.edit_message_text(chat_id=chat_id, message_id=cq.get("message",{}).get("message_id"),
                                          text=msg, reply_markup=build_fng_keyboard(lang_cq))
                except Exception:
                ...
                lang_cq = get_lang_override(chat_id) or DEFAULT_LANG
                msg = format_btc_dominance(fetch_btc_dominance(), lang_cq)
                try:
                    bot.edit_message_text(chat_id=chat_id, message_id=cq.get("message",{}).get("message_id"),
                                          text=msg, reply_markup=build_btcdom_keyboard(lang_cq))
                except Exception:
            ...
            try: bot.answer_callback_query(cq.get("id"), text="Error", show_alert=False)
            except Exception: pass
        return "ok"
    ...
    if t_low in ("/start","start"):
        ...
        if not DONATE_STICKY: send_donate_message(chat_id, start_lang)
        return "ok"
    ...
    if t_low in ("/donate","donate","донат","/tip","tip"):
        send_donate_message(chat_id, lang); return "ok"

    # -------- Команды с и без слэша --------
    # /price ...  |  "price ..." (без слэша)
    if t_low.startswith("/price") or t_low.startswith("price "):
    ...
    # /top10  |  "top10"
    if t_low.startswith("/top10") or t_low == "top10":
    ...
    # /feargreed | /fng | "feargreed" | "fng"
    if t_low.startswith("/feargreed") or t_low == "/fng" or t_low == "feargreed" or t_low == "fng":
        msg_out = format_fear_greed(fetch_fear_greed(), lang)
    ...
    if t_low.startswith("/btcdom") or t_low == "btcdom":
        msg_out = format_btc_dominance(fetch_btc_dominance(), lang)
    ...
    if m:
        address = m.group(0)
        report = format_report(analyze_eth_contract(address), lang)
        bot.send_message(chat_id=chat_id, text=report, reply_markup=build_donate_keyboard() if DONATE_STICKY else None)
        return "ok"

    # Быстрые цены по триггерам из текста
    ...
    bot.send_message(chat_id=chat_id, text=answer, reply_markup=build_donate_keyboard() if DONATE_STICKY else None)
    return "ok"
