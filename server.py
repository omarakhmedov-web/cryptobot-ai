...
def webhook_with_secret(secret):
    ...
    if (
        ...
        chg = "24h"
        mkts = coingecko_top_market(10, chg)
        msg_out, ids = format_top10(mkts, lang=cur_lang, chg=chg)
        bot.send_message(chat_id=chat_id, text=msg_out, reply_markup=build_top10_keyboard(chat_id, ids, cur_lang, chg=chg))
        return "ok"

# Optional text aliases for timeframes (avoid AI fallback if users type "/24h" etc.)
if t_low in ("/1h", "1h"):
    chg = "1h"
    mkts = coingecko_top_market(10, chg)
    msg_out, ids = format_top10(mkts, lang=cur_lang, chg=chg)
...
if t_low in ("/7d", "7d", "/30d", "30d", "/1y", "1y"):
    chg = t_low.strip("/")
    mkts = coingecko_top_market(10, chg)
    msg_out, ids = format_top10(mkts, lang=cur_lang, chg=chg)
    bot.send_message(chat_id=chat_id, text=msg_out, reply_markup=build_top10_keyboard(chat_id, ids, cur_lang, chg=chg))
